...
function Header({ wallpaper }) {
  return (
    <div className="w-full h-[65vh] relative overflow-hidden">
      ...
      <p className="text-md text-white tracking-tighter w-1/2">{wallpaper.overview.slice(0,200)}<Link className="text-md font-bold text-[#6556CC]">...more</Link> </p>
      <div className="flex items-center gap-5">
        
      <p><i className="ri-megaphone-fill text-yellow-300"></i>  {wallpaper.release_date || 'No Information'}</p>
      <p><i className="ri-album-fill text-yellow-300"></i>  {wallpaper.media_type.toUpperCase()}</p>
      </div>
